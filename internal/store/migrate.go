...
func (s *FileStore) migrateRecords() error {
	...
	for _, entry := range entries {
		...
		if version == legacySchemaVersion {
			...
			if prefixedName != record.Name {
				...
				if err := model.Validate(&record); err != nil {
					return fmt.Errorf("normalize tunnel record %s name from %q to %q: %w", id, previousName, prefixedName, err)
				}
				needsRewrite = true
			}
		}
		...
		if owner, exists := names[record.Name]; exists {
			return fmt.Errorf("tunnel name %q is shared by records %s and %s after migration", record.Name, owner, id)
		}
		names[record.Name] = id
		if needsRewrite {
			plans = append(plans, recordMigrationPlan{
				id: id, record: record, from: version, previousName: previousName,
			})
		}
	}
...
func migrateSchema2(data []byte) (model.Tunnel, error) {
	...
	if err := model.Validate(&record); err != nil {
		return model.Tunnel{}, fmt.Errorf("validate migrated record: %w", err)
	}
	return record, nil
}
	...
	for _, source := range legacy.Sources {
		...
		for _, family := range families {
			...
			migrated.Sources = append(migrated.Sources, model.SRv6Source{
				...
				SID: *family.sid, Priority: 0, MTU: source.MTU,
			})
		}
	}
	return migrated, nil
